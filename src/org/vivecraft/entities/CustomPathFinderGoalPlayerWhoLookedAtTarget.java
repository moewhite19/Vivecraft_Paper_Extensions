...
public class CustomPathFinderGoalPlayerWhoLookedAtTarget extends NearestAttackableTargetGoal<Player> {
    ...
    public CustomPathFinderGoalPlayerWhoLookedAtTarget(EnderMan entityenderman,TargetingConditions.Selector p) {
        super(entityenderman,Player.class,10,false,false,p);
        this.enderman = entityenderman;
        this.isAngerInducing = (entityliving) -> {
            return (isLookingAtMe((Player) entityliving) || entityenderman.isAngryAt(entityliving,entityliving.level().getMinecraftWorld())) && !entityenderman.hasIndirectPassenger(entityliving);
        };
        this.startAggroTargetConditions = TargetingConditions.forCombat().range(this.getFollowDistance()).selector((livingEntity,serverLevel) -> {
            return this.isAngerInducing.test(livingEntity);
        });
    }

    public boolean canUse() {
        this.pendingTarget = this.enderman.level().findNearbyPlayer(this.enderman,64,entity -> {
            if (entity instanceof LivingEntity livingEntity){
                return this.startAggroTargetConditions.test(this.enderman.level().getMinecraftWorld(),enderman,livingEntity);
            }
            return false;
        });
    ...
    public boolean canContinueToUse() {
        if (this.pendingTarget != null){
            ...
            if (this.target != null){
                ...
                if (this.continueAggroTargetConditions.test(enderman.level().getMinecraftWorld(),this.enderman,this.target)){
                    return true;
                }
            }
