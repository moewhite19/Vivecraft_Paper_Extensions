...
public class VivecraftCombatListener implements Listener {
    ...
    public void onProjectileLaunch(ProjectileLaunchEvent event) {
        ...
        Location pos = vp.getControllerPos(vp.activeBodyPart);
        Vec3 aim = vp.getControllerDir(vp.activeBodyPart);
        ...
        if (arrow && vp.getDraw() != 0){
            ...
            if (!vp.isSeated()){ //standing
                pos = vp.getControllerPos(BodyPart.MAIN_HAND);
                Vector m = (vp.getControllerPos(BodyPart.OFF_HAND).subtract(vp.getControllerPos(BodyPart.MAIN_HAND))).toVector();
                m = m.normalize();
