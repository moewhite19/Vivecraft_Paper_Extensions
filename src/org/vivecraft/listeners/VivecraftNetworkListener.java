...
*/
            ...
            case VERSION:
                ...
                sender.sendPluginMessage(vse,VSE.CHANNEL,StringToPayload(PacketDiscriminators.VERSION,vse.getDescription().getFullName())); //发送插件版本
//                sender.sendPluginMessage(vse,VSE.CHANNEL,new byte[]{(byte) PacketDiscriminators.NETWORK_VERSION.ordinal(),1}); //发送协议版本1?
                sender.sendPluginMessage(vse,VSE.CHANNEL,vse.getServerNetworkVersionRaw()); //发送协议版本
                ...
                try{
                    ...
                    if (vse.getClimbeyBlocksRaw() != null){
                        sender.sendPluginMessage(vse,VSE.CHANNEL,vse.getClimbeyBlocksRaw());
...
//                        sender.sendPluginMessage(vse,VSE.CHANNEL,p);
                    ...
                    //发送不允许双持的消息
//                    sender.sendPluginMessage(vse,VSE.CHANNEL,new byte[]{(byte) PacketDiscriminators.DUAL_WIELDING.ordinal()});
            ...
            case VR_PLAYER_STATE:
                //把data数组转换成FriendlyByteBuf,并用VRPlayerStatePayloadC2S:read()读取
                FriendlyByteBuf buffer = new FriendlyByteBuf(Unpooled.wrappedBuffer(data));
//                VrPlayerState.deserialize(buffer,0);
                vp.updateState(VrPlayerState.deserialize(buffer,0));
                break;
            case DUAL_WIELDING:
                //don't care yet.
            default:
